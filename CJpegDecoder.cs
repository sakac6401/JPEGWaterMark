...
{
    public class CJpegDecoder
    {
        const int DC = 0;
        const int AC = 1;
        const int EOB = 0x00;
        public Cjpeg cj = null;
        int[] t_sel = { 0, 1, 1 };
        ...
        public void HuffmanDecode()
        {
            ...
            for (int i = 0; i < (cj.cb.block_width * cj.cb.block_height); i++)
            {
                ...
                {
                    //DC
                    c_value = GetValueLength(t_sel[k], DC);
                    cj.cb.data[k][i][0] = GetValue(c_value);

                    //AC
                    for (int j = 1; j < 64;j++)
                    {
                        c_value = GetValueLength(t_sel[k], AC);
                        ...
                        else
                        {
                            ...
                            cj.cb.data[k][i][j] = GetValue(v_length);
        ...
        /// </summary>
        /// <returns></returns>
        public int GetValueLength(int t_sel, int ac_dc)
        {
            int seek=0;
            int RL;
            while (true)
        ...
        public int GetValue(int v_length)
        {
            if (v_length == 0)
            {
                return 0;
